/////////////////////////////////////////////////////////////////////
//MISSION 20: THE ESSENCE OF THE SOURCE (XP: 1175 / 1500)
...
//FEEDBACK:
//noted that your global vairable is not reset to 0 everytime you run it. 
//Still works though!
...
parse("function foo(n) {n + 1;} foo(0); foo(0); foo(0);");


//FEEDBACK:
//not exactly right
//you never account for lamda expressions. 
//function delcarations are a subset of lamda expressions. 
//Hence, you will miss counting some lamda declarations.
/*Actual answer:

// Changes to makefunction function function makefunction(parameters, body, env) { funcobjcount = funcobjcount + 1; // new return list("compound_function", parameters, body, env); }
*/
...
//TEST CASE
display_list(parse_and_evaluate(`1 + 2;`));
...
// evaluation
//
function mce_map(f, x){
    return is_null(x)
            ? x
            : pair(apply(f, head(x)), mce_map(f, tail(x)));
}

function more_than(a, b){
    return is_compound_function(a) || is_primitive_function(a)
            ? mce_map(a, b)
            : a > b;
}
